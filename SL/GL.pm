...
sub delete_transaction {
    ...
    my %defaults =
      $form->get_defaults( $dbh, \@{ [ 'precision', 'extendedlog' ] } );

    if ( $form->{id} and $defaults{extendedlog} ) {
        $query =
qq|INSERT INTO gl_log_deleted SELECT * FROM gl WHERE id = $form->{id}|;
    ...
    if ( $form->{batchid} *= 1 ) {
        ...
        $form->update_balance( $dbh, 'br', 'amount', qq|id = $form->{batchid}|,
            $amount );
...
sub post_transaction {
    ...
    if ( $form->{id} *= 1 and $defaults{extendedlog} ) {
        ...
        $query =
          qq|UPDATE gl SET ts = NOW() + TIME '00:00:01' WHERE id = $form->{id}|;
        $dbh->do($query) || $form->dberror($query);
    }
    ...
    for $i ( 1 .. $form->{rowcount} ) {
        ...
        if ($credit) {
            $amount = $credit;
            $bramount += $form->round_amount( $amount * $exchangerate,
                $form->{precision} );
        }
        if ($debit) {
            $amount    = $debit * -1;
        ...
        if ($keepcleared) {
            $cleared = $form->dbquote( $form->dbclean( $form->{"cleared_$i"} ),
                SQL_DATE );
        }
            ...
            || ( $project_id ne 'NULL' ) )
        {
            ...
            ($tax_chart_id) = $dbh->selectrow_array(
		  ...
		  $cleared, '$approved', '$form->{"tax_$i"}', $tax_chart_id, $taxamount)|;
            ...
            if ( $form->{currency} ne $form->{defaultcurrency} ) {
                ...
                $amount = $form->round_amount( $amount * ( $exchangerate - 1 ),
                    $form->{precision} );
...
qq|INSERT INTO acc_trans (trans_id, chart_id, amount, transdate,
   ...
   # Fix for rounding diff: https://github.com/ledger123/runmyaccounts/issues/67
    if ( $form->{currency} ne $form->{defaultcurrency} ) {
        $query =
"SELECT SUM(amount) FROM acc_trans WHERE trans_id = $form->{id} AND fx_transaction";
        my ($diff) = $dbh->selectrow_array($query);
        $diff = abs($diff);
        if ($diff) {
            my ($entry_id) = $dbh->selectrow_array(
"SELECT entry_id FROM acc_trans WHERE trans_id = $form->{id} AND fx_transaction AND amount > 0 LIMIT 1"
            );
            $dbh->do(
"UPDATE acc_trans SET amount = amount + $diff WHERE entry_id = $entry_id"
            );
        }
    }
...
sub transactions {
    ...
    my ( $glwhere, $arwhere, $apwhere ) =
      ( "g.approved = '1'", "a.approved = '1'", "a.approved = '1'" );
    ...
    if ( $form->{accnofrom} ) {
        ...
        ( $form->{accnofrom_description}, $form->{accnofrom_translation} ) =
          $dbh->selectrow_array($query);
        $form->{accnofrom_description} = $form->{accnofrom_translation}
          if $form->{accnofrom_translation};
    ...
    if ( $form->{accnoto} ) {
        ...
        ( $form->{accnoto_description}, $form->{accnoto_translation} ) =
          $dbh->selectrow_array($query);
        $form->{accnoto_description} = $form->{accnoto_translation}
          if $form->{accnoto_translation};
        ...
        if ( $form->{accno} ) {
		  ...
		  WHERE c.accno = | . $dbh->quote( $form->{accno} );
            (
                ...
                $form->{account_translation}
            ) = $dbh->selectrow_array($query);
            $form->{account_description} = $form->{account_translation}
              if $form->{account_translation};
        }
...
# CREATE TABLE filtered (trans_id int, entry_id int default 0, entry_id2 int default 0, debit float default 0, credit float default 0);
     ...
     |;
        $query .= " AND ac.transdate >= '$form->{datefrom}'"
          if $form->{datefrom};
    ...
    while ( my $ref = $sth->fetchrow_hashref(NAME_lc) ) {
        ...
        for (qw(address1 address2 city zipcode country)) {
            $ref->{address} .= "$ref->{$_} ";
        }
    ...
    for my $id ( keys %trans ) {
        ...
        for $i (
            reverse sort { $trans{$id}{$a}{amount} <=> $trans{$id}{$b}{amount} }
            keys %{ $trans{$id} }
          )
        {
        ...
        if ($aa) {
            ...
            for (@arap) {
                $i = 0;
                for $ref (@accno) {
                    $form->{GL}[$_]{contra} .= "$ref->{accno} "
                      unless $seen{ $ref->{accno} };
                    $seen{ $ref->{accno} } = 1;
                    $form->{GL}[$_]{gifi_contra} .= "$ref->{gifi_accno} "
                      unless $seen{ $ref->{gifi_accno} };
            ...
            for $i (
                reverse
                sort { $trans{$id}{$a}{amount} <=> $trans{$id}{$b}{amount} }
                keys %{ $trans{$id} }
              )
            {
                $found      = 0;
                ...
                if ( $trans{$id}{$i}{$amt} ) {
                    for ( keys %accno ) {
                        if ( $accno{$_}{$rev} == $trans{$id}{$i}{$amt} ) {
                            ...
                            $form->{GL}[$_]{gifi_contra} =
                              $trans{$id}{$i}{gifi_accno};
                            $found = 1;
                            last;
                        }
                    }
                ...
                if ( !$found ) {
                    ...
                    if ($amount) {
                        for $i (
                            reverse sort { $a{amount} <=> $b{amount} }
                            keys %accno
                          )
                        {
                            if ( $accno{$i}{amount} <= $amount ) {
                                ...
                                $amount = $form->round_amount(
                                    $amount - $accno{$i}{amount}, 10 );
                                last if $amount < 0;

                                $form->{GL}[$j]{contra} .= "$accno{$i}{accno} "
                                  unless $seen{ $accno{$i}{accno} };
                                $seen{ $accno{$i}{accno} };
                                $form->{GL}[$j]{gifi_contra} .=
                                  "$accno{$i}{gifi_accno} "
                                  unless $seen{ $accno{$i}{gifi_accno} };
...
sub transaction {
    ...
    my %defaults =
      $form->get_defaults( $dbh, \@{ [qw(closedto revtrans precision)] } );
    for ( keys %defaults ) { $form->{$_} = $defaults{$_} }
    ...
    if ( $form->{id} *= 1 ) {
        ...
        if ( $fxdr < 0 || $fxcr > 0 ) {
            $form->{fxadj} = 1
              if $form->round_amount( $fxdr * -1, $form->{precision} ) !=
              $form->round_amount( $fxcr, $form->{precision} );
        }
